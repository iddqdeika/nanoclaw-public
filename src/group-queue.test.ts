...
describe('GroupQueue', () => {
  ...
  // Layer 2 (group-queue's own retry-with-backoff) was removed when the
  // durable recovery system in src/recovery-sweep.ts took over. The
  // queue's processGroupMessages callback no longer signals failure; it
  // records error state to the sessions row and returns void. Tests for
  // that behaviour live in src/recovery.test.ts now.
