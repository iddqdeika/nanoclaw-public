...
vi.mock('@slack/bolt', () => ({
  App: class MockApp {
    ...
    client = {
      ...
      chat: {
        // Production code reads `resp.ts` after postMessage. Returning
        // `undefined` (the old mock) crashed the split-loop after the
        // first call. Mirror a realistic Slack response so the loop
        // completes all iterations.
        postMessage: vi.fn().mockResolvedValue({ ts: '1704067200.000000' }),
      },
      conversations: {
...
describe('SlackChannel', () => {
  ...
  describe('@mention translation', () => {
    // Removed: prepending @<assistant> to @-mention messages was removed
    // in commit a3a4598 "fix(slack): remove prepending of @ASSISTANT_NAME
    // to mentioned messages". The triggering decision happens elsewhere
    // (trigger_pattern match in message loop) — channel just passes the
    // text through unchanged. The test below verifies pass-through holds.

    it('passes @mention messages through unchanged (no auto-prepend)', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await channel.connect();
  ...
  describe('syncChannelMetadata pagination', () => {
    it('paginates through multiple pages of channels', async () => {
      ...
      await channel.connect();
      // syncChannelMetadata is fired-and-forgotten by connect(); let
      // microtasks settle so both pages complete before assertions.
