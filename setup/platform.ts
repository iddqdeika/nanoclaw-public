...
export function commandExists(name: string): boolean {
  // Windows uses `where`, POSIX uses `command -v`. Detect via platform.
  // `command -v` is a shell builtin (not an exe), so it needs shell:true
  // on POSIX; `where` is a real binary on Windows.
