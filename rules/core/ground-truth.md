...
| Task was scheduled | Check `mcp__nanoclaw__list_tasks` response |
| Tool call succeeded | Check the tool return value |
| DB state | Query `/workspace/project/store/messages.db` (main only) |
...
LLMs synthesize plausible-sounding answers from prior context. This produces confident, wrong reports. Whether the question is about scheduled tasks, file contents, rules, skills, or past actions — the model's memory of what *should* be there is not the same as what *is* there.
